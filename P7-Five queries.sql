...
FROM Customer c
JOIN Pet p ON c.customerID = p.petID
...
SELECT c.firstName, c.lastName, COUNT(o.orderID) AS orderCount, AVG(o.totalAmount) AS avgTotalAmount
FROM Customer c
JOIN "Order" o ON c.customerID = c.customerID
GROUP BY c.customerID, c.firstName, c.lastName
...
SELECT c.firstName, c.lastName,
       CASE
           WHEN (SELECT COUNT(*) FROM Pet p WHERE p.petID = c.customerID) = 1 THEN 'Single Pet Owner'
           WHEN (SELECT COUNT(*) FROM Pet p WHERE p.petID = c.customerID) > 1 THEN 'Multiple Pet Owner'
           ELSE 'No Pets'
